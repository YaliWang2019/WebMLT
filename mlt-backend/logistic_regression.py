...
def lgrRmMissingvalues(id):
  df = csv_file[id]
  df_new = df.dropna()
  df_preview = df_new[0:5]
  return ((df_preview.to_json()), 200)

def lgrScaling(id, scaleMode):
  df = csv_file[id]
  df_new = df.dropna()
  X = df.iloc[:, 2:]
  y = df.iloc[:, 1]
...
def lgrScatterImg(id, scaleMode):
  (X, Y, y) = lgrScaling(id, scaleMode)
  plt.clf()
  ...
  return (json.dumps({'lgrImgScatter':img_to_base64(plt)}), 200)


  # Split the Dataset into Training and Test Set
def lgrSpliting(id, test_size=0.2, random_state=0, scaleMode="normalization"):
  (X, Y) = lgrScaling(id, scaleMode)
...
def lgr_train_test_imgs(id, test_size, random_state):
  (X_train_split, X_test_split, Y_train, Y_test) = lgrSpliting(id, float(test_size), int(random_state))
  plt.clf()
...
def lgr_pre_train(id, test_size, random_state):
  (X_train_split, X_test_split, Y_train, Y_test) = lgrSpliting(id, test_size, random_state)
  ...
  # plt.tight_layout()
def lgrModelTraining(id, test_size, random_state):
...
def lgrConfusionMatrix(cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
  if normalize:
...
def lgrMakeConfusionMatrix(id, test_size, random_state):
  (_, _, _, _, _, _, Y_test, Y_pred) = lgr_pre_train(id, float(test_size), int(random_state))
  cm = confusion_matrix(Y_test, Y_pred)
  labels = ["+", "-"]
  matrix = lgrConfusionMatrix(cm, labels)  
